...
create or replace function public.guard_manual_offer_price()
returns trigger language plpgsql set search_path = '' as $function$
begin
  if new.manual_selling_price_usd is not null and new.manual_selling_price_usd < new.supplier_price_usd then
    raise exception 'MANUAL_PRICE_BELOW_SUPPLIER_COST';
  end if;
  return new;
end;
$function$;
drop trigger if exists guard_manual_offer_price on public.store_product_offers;
create trigger guard_manual_offer_price before insert or update of supplier_price_usd, manual_selling_price_usd on public.store_product_offers for each row execute function public.guard_manual_offer_price();

drop function if exists public.admin_update_platform_pricing_v2(numeric,numeric,numeric,text,numeric,numeric,numeric,boolean,integer,numeric,numeric,numeric);
create or replace function public.admin_update_platform_pricing_v2(
 p_usd_to_egp_rate numeric,p_profit_per_usd_egp numeric,p_minimum_profit_egp numeric,p_api_pricing_mode text,
 p_default_profit_usd numeric,p_default_markup_percentage numeric,p_low_supplier_balance_usd numeric,p_auto_disable_over_balance boolean,
 p_points_per_usd integer,p_egp_deposit_fee_per_1000 numeric,p_egp_deposit_minimum_fee numeric,p_usd_deposit_fixed_fee numeric
) returns jsonb
...
begin
 ...
 if p_usd_to_egp_rate<=0 then raise exception 'Invalid USD exchange rate';end if;
 if p_api_pricing_mode not in('fixed_usd','percentage','manual') then raise exception 'Invalid API pricing mode';end if;
 if least(p_profit_per_usd_egp,p_minimum_profit_egp,p_default_profit_usd,p_default_markup_percentage,p_low_supplier_balance_usd,p_points_per_usd,p_egp_deposit_fee_per_1000,p_egp_deposit_minimum_fee,p_usd_deposit_fixed_fee)<0 then raise exception 'Pricing values cannot be negative';end if;
 select to_jsonb(s) into v_before from public.platform_settings s where id=1 for update;
 if v_before is null then raise exception 'Platform settings are missing';end if;
 update public.platform_settings s set usd_to_egp_rate=p_usd_to_egp_rate,profit_per_usd_egp=p_profit_per_usd_egp,minimum_profit_egp=p_minimum_profit_egp,api_pricing_mode=p_api_pricing_mode,default_profit_usd=p_default_profit_usd,default_markup_percentage=p_default_markup_percentage,low_supplier_balance_usd=p_low_supplier_balance_usd,auto_disable_over_balance=p_auto_disable_over_balance,points_per_usd=p_points_per_usd,egp_deposit_fee_per_1000=p_egp_deposit_fee_per_1000,egp_deposit_minimum_fee=p_egp_deposit_minimum_fee,usd_deposit_fixed_fee=p_usd_deposit_fixed_fee,pricing_updated_at=now(),updated_at=now() where id=1 returning to_jsonb(s) into v_after;
 insert into public.activity_logs(user_id,actor_id,action,entity_type,entity_id,description,old_data,new_data) values(v_admin.id,v_admin.id,'platform_pricing_updated','platform_settings',null,'Admin updated platform pricing',v_before,v_after);
 return jsonb_build_object('success',true,'usd_to_egp_rate',v_after->'usd_to_egp_rate','profit_per_usd_egp',v_after->'profit_per_usd_egp','minimum_profit_egp',v_after->'minimum_profit_egp','api_pricing_mode',v_after->'api_pricing_mode','default_profit_usd',v_after->'default_profit_usd','default_markup_percentage',v_after->'default_markup_percentage','low_supplier_balance_usd',v_after->'low_supplier_balance_usd','auto_disable_over_balance',v_after->'auto_disable_over_balance','points_per_usd',v_after->'points_per_usd','egp_deposit_fee_per_1000',v_after->'egp_deposit_fee_per_1000','egp_deposit_minimum_fee',v_after->'egp_deposit_minimum_fee','usd_deposit_fixed_fee',v_after->'usd_deposit_fixed_fee','pricing_updated_at',v_after->'pricing_updated_at');
end;$function$;
revoke all on function public.admin_update_platform_pricing_v2(numeric,numeric,numeric,text,numeric,numeric,numeric,boolean,integer,numeric,numeric,numeric) from public;
grant execute on function public.admin_update_platform_pricing_v2(numeric,numeric,numeric,text,numeric,numeric,numeric,boolean,integer,numeric,numeric,numeric) to authenticated;
...
begin
  ...
  loop
    ...
    if v_offer.supplier_price_usd < 0
       ...
       or (v_offer.manual_selling_price_usd is not null and v_offer.manual_selling_price_usd < v_offer.supplier_price_usd)
       or coalesce(v_offer.manual_selling_price_usd, v_offer.supplier_price_usd + v_offer.profit_usd) <= 0 then
    ...
    v_unit_price_usd := round(
      coalesce(v_offer.manual_selling_price_usd, v_offer.supplier_price_usd + v_offer.profit_usd),
      8
    );
  ...
  loop
    ...
    insert into public.product_order_items (
      ...
      v_unit_price_usd - v_offer.supplier_price_usd, v_unit_price_usd, v_item_total_usd,
...
begin
  ...
  v_unit_price := round(coalesce(v_offer.manual_selling_price_usd, v_offer.supplier_price_usd + v_offer.profit_usd), 8);
  ...
  insert into public.product_order_items (
    ...
    p_quantity, v_offer.supplier_price_usd, v_unit_price - v_offer.supplier_price_usd, v_unit_price,
...
create or replace function public.admin_reprice_provider_offers_v2()
returns integer language plpgsql security definer set search_path='' as $function$
declare v_actor uuid:=auth.uid();v_role text;v_settings public.platform_settings;v_count integer:=0;
begin
  select role::text into v_role from public.profiles where id=v_actor and status::text='active';
  if v_actor is null or v_role not in('admin','super_admin','owner') then raise exception 'Admin permission required';end if;
  select * into v_settings from public.platform_settings where id=1;
  ...
  where id is not null and provider_name is not null;
  get diagnostics v_count=row_count;
  insert into public.activity_logs(user_id,actor_id,action,entity_type,description,new_data) values(v_actor,v_actor,'provider_offers_bulk_repriced','store_product_offer',format('Repriced %s provider offers',v_count),jsonb_build_object('offers_count',v_count,'pricing_mode',v_settings.api_pricing_mode));
  return v_count;
end;$function$;
revoke all on function public.admin_reprice_provider_offers_v2() from public;
grant execute on function public.admin_reprice_provider_offers_v2() to authenticated;
