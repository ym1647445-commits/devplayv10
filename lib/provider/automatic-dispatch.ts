...
interface Job {id:string;provider_code:string|null;order_id:string;order_item_id:string;product_id:string|null;offer_id:string|null;provider_offer_id:string|null;supplier_product_id:string|null;input_values:Record<string,string>|null;attempts_count:number}
...
export async function dispatchCreatedOrder(orderId:string):Promise<void>{
  ...
  const{data:jobs,error}=await db.from("product_supplier_jobs").select("id,provider_code,order_id,order_item_id,product_id,offer_id,provider_offer_id,supplier_product_id,input_values,attempts_count").eq("order_id",orderId).eq("status","pending").order("created_at",{ascending:true}).returns<Job[]>();
  if(error)throw error;
  for(const job of jobs??[]){
    // Item4Gamer must only be dispatched by the static-IP VPS worker.
    if(job.provider_code==="item4gamer")continue;
...
export async function syncAllSupplierStatuses():Promise<{processed:number}> {
  ...
  const db=createClient(url,key,{auth:{persistSession:false,autoRefreshToken:false}});
  const{data:jobs,error}=await db.from("product_supplier_jobs").select("id,provider_code,order_id,order_item_id,product_id,offer_id,provider_offer_id,supplier_product_id,input_values,attempts_count,supplier_order_id").in("status",["supplier_pending","failed","completed"]).not("supplier_order_id","is",null).limit(100);
  if(error)throw error;const safeJobs=(jobs??[]).filter(row=>row.provider_code!=="item4gamer");if(!safeJobs.length)return{processed:0};
  const supplierIds=new Set(safeJobs.map(row=>String(row.supplier_order_id)));const byId=await providerOrdersForJobs(supplierIds);let processed=0;
  for(const row of safeJobs){const match=byId.get(String(row.supplier_order_id));if(!match)continue;const succeeded=["completed","success"].includes(match.status);const refunded=match.status==="refunded";const failed=["failed","cancelled","rejected"].includes(match.status);if(!succeeded&&!refunded&&!failed)continue;
