...
type Offer={id:string;product_id:string;provider_offer_id:string|null;name_ar:string;name_en:string|null;supplier_price_usd:number|string;profit_usd:number|string;stock:number|null;active:boolean;available:boolean;required_fields:ProductRequiredField[]|null;provider_data:Record<string,unknown>|null};
...
export function ManualOrderForm({customers,products,rate,defaultCustomer,canComplimentary}:{customers:Customer[];products:Product[];rate:number;defaultCustomer:string;canComplimentary:boolean}){
  ...
  <div className={styles.layout}><section className={styles.form}>
    ...
    <Step icon={<PackagePlus/>} title="2. المنتج والباقة"><div className={styles.two}><label>المنتج<select value={productId} onChange={e=>selectProduct(e.target.value)}><option value="">اختاري المنتج</option>{products.map(p=><option key={p.id} value={p.id}>{p.name_ar}</option>)}</select></label><label>الباقة<select value={offerId} onChange={e=>{setOfferId(e.target.value);setInputs({});setResult(null)}} disabled={!product}><option value="">اختاري الباقة</option>{offers.map(o=><option key={o.id} value={o.id}>{o.name_ar} — ${(Number(o.supplier_price_usd)+Number(o.profit_usd)).toFixed(4)}</option>)}</select></label></div><label>الكمية<input type="number" min={product?.minimum_quantity??1} max={product?.maximum_quantity??100} value={quantity} onChange={e=>setQuantity(Number(e.target.value))}/></label></Step>
