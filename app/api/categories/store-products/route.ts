...
interface OfferRow { supplier_price_usd:number|string;profit_usd:number|string;active:boolean;available:boolean }
...
export async function GET(){
  ...
  const result=(products??[]).map((product)=>{
    ...
    const prices=offers.map((offer)=>(Number(offer.supplier_price_usd)+Number(offer.profit_usd))*rate).filter(Number.isFinite);
