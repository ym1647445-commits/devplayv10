...
export default async function SearchPage({searchParams}:{searchParams:Promise<{q?:string}>}){
  ...
  const[{data},{data:{user}}]=await Promise.all([
    supabase.from("store_products").select("id,slug,name_ar,name_en,short_description_ar,image_url,badge,featured,instant_delivery,delivery_time,rating,reviews_count,status,category:store_categories(name_ar),store_product_offers(id,name_ar,name_en,supplier_price_usd,profit_usd,manual_selling_price_usd,active,available,stock)").eq("active",true).neq("status","unavailable").order("featured",{ascending:false}).order("created_at",{ascending:false}).returns<Row[]>(),
    supabase.auth.getUser(),
  ]);
  const products:Product[]=(data??[]).flatMap(row=>{const offers=(row.store_product_offers??[]).filter(offer=>offer.active&&offer.available&&(offer.stock===null||offer.stock>0));if(!offers.length)return[];const effective=(offer:typeof offers[number])=>offer.manual_selling_price_usd===null?Number(offer.supplier_price_usd)+Number(offer.profit_usd):Number(offer.manual_selling_price_usd);const lowest=offers.reduce((best,offer)=>effective(offer)<effective(best)?offer:best);return[{id:row.id,slug:row.slug,name:row.name_ar,category:relation(row.category)?.name_ar??"ألعاب وخدمات",image:row.image_url??"/devplay-icon.svg",shortDescription:row.short_description_ar??undefined,supplierPriceUsd:Number(lowest.supplier_price_usd),profitUsd:Number(lowest.profit_usd),manualSellingPriceUsd:lowest.manual_selling_price_usd===null?null:Number(lowest.manual_selling_price_usd),costPrice:Number(lowest.supplier_price_usd),price:effective(lowest),currency:"USD" as const,rating:Number(row.rating??5),reviewsCount:row.reviews_count??0,badge:row.badge??undefined,status:row.status,featured:row.featured,instantDelivery:row.instant_delivery,deliveryTime:row.delivery_time??undefined,requiredFields:[],providerData:{mainProductId:row.id,searchTerms:[row.name_en??"",...offers.flatMap(offer=>[offer.name_ar,offer.name_en??""])]}}]});
